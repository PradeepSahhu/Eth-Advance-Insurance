...
contract CarInsurance is ICarInsurance{
   ...
   // Initial Amount : 8000 wei
    ...
    constructor(address _storeBank ,address _owner, uint _age, uint8 _insuranceType, uint _initialAmount, bool _pastAccident){
        StoreBank = payable(_storeBank);
        owner = _owner;
        uint _perMonthPremiumAmount = _age.carInsuranceCalculatePremium(_pastAccident, _insuranceType);
        customerDetails = customerInfo({customerAddress: owner, customerAge:_age, perMonthPremiumAmount:_perMonthPremiumAmount, insuranceType : _insuranceType, totalDepositedAmount : _initialAmount, claimed: false });
    }


    function addPremium() public payable onlyOwner{
        require(msg.value > 0, "No amount is submitted");
        require(msg.value >= customerDetails.perMonthPremiumAmount, "Premium is not equal to the promised amount");
        uint bal = msg.value - customerDetails.perMonthPremiumAmount;
        monthCount++;
        customerDetails.totalDepositedAmount += customerDetails.perMonthPremiumAmount;
        (bool callMsg,) = StoreBank.call{value: customerDetails.perMonthPremiumAmount}("");
    ...
    function claimInsurance() public payable onlyOwner{
        require(customerDetails.claimed == false, "You already claimed the insurance");
            uint claimAmount = customerDetails.insuranceType == 1? 1000000: 10000000;
       ...
       bool claimedStatus =  IinsuranceFactory(StoreBank).claimedInsurance(owner, claimAmount, address(this));
